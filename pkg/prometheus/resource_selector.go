...
func (lcv *LabelConfigValidator) validate(rc monitoringv1.RelabelConfig) error {
	...
	isValidLabelNameWithRegexVarFn := func(value string) bool {
		// UTF-8 validation allows $ and ${<...>} for variables, but relabelling configs cannot use $ and ${<...>} characters.
		return lcv.isValidLabelName(value)
	}
	if (action == string(relabel.Replace)) && varInRegexTemplate(rc.TargetLabel) && !isValidLabelNameWithRegexVarFn(rc.TargetLabel) {
		return fmt.Errorf("%q is invalid 'target_label' for %s action", rc.TargetLabel, rc.Action)
	}
	...
	if action == string(relabel.LabelMap) && (rc.Replacement != nil) && !isValidLabelNameWithRegexVarFn(*rc.Replacement) {
